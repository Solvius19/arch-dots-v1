...
Item {
    ...
    Process {
        command: ["bash", "-c", "source ~/.local/state/arch-dots-v1-version 2>/dev/null && [ -n \"$LOCAL_VERSION\" ] && echo $LOCAL_VERSION || echo '0.0.0'"]
        running: true
    ...
    Process {
        command: ["bash", "-c", "curl -m 5 -s https://raw.githubusercontent.com/Solvius19/arch-dots-v1/master/install.sh | grep '^DOTS_VERSION=' | cut -d'\"' -f2"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text ? this.text.trim() : "";
    ...
    Process {
        command: ["bash", "-c", "curl -m 5 -sL \"https://api.github.com/repos/Solvius19/arch-dots-v1/commits/master\" | grep -m1 '\"message\":' | cut -d'\"' -f4 || echo 'No changelog available'"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text ? this.text.trim() : "";
            ...
            Rectangle {
                ...
                NumberAnimation {
                    ...
                    onFinished: {
                        ...
                        let cmd = "if command -v kitty >/dev/null 2>&1; then kitty --hold bash -c 'eval \"$(curl -fsSL https://raw.githubusercontent.com/Solvius19/arch-dots-v1/master/install.sh)\"'; else ${TERM:-xterm} -hold -e bash -c 'eval \"$(curl -fsSL https://raw.githubusercontent.com/Solvius19/arch-dots-v1/master/install.sh)\"'; fi";
